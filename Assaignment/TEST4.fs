...
type environment (boardWidth : int, NMooses : int, mooseRepLen : int, NWolves : int, wolvesRepLen : int, wolvesHungLen : int, verbose : bool) =
  ...
  member this.tick () =
    ...
    let listw = (List.toArray _board.wolves)
    let listm = (List.toArray _board.moose)
    ...
    for i = 0 to _board.moose.Length - 1 do
      for j = 0 to _board.wolves.Length - 1 do
        ...
        then
        //elg spises
          wolf.position <- Some (_board.moose.[i].position.Value)
          listw.[j] <- wolf
    ...
    let _board.wolves = (Array.ofList listw)
    let _board.moose = (Array.ofList listm)
    0
